...
namespace Author_Sorting {
	...
	inline void sort(vector<authors>& a, DWORD s, DWORD t) {
		...
		for (int i = 0; i < a.size(); i++) {
			if (a[i].weight < 100000)
				bucket[a[i].weight].push_back(a[i].name);
			else
				bucket[100000].push_back(a[i].name);
		}
		a.clear();
		for (int i = 100000; i >= 0; i--) {
			for (int j = 0; j < bucket[i].size(); j++) {
				authors tmp(bucket[i][j], i);
				a.push_back(tmp);
			}
		}
		return;
	...
	bool Author_Sort(void)
	{
		HANDLE check_author_database_handle = CreateFile(L"database\\author_rank", FILE_READ_EA, FILE_SHARE_READ, 0, OPEN_EXISTING, 0, 0);
		if (check_author_database_handle == INVALID_HANDLE_VALUE) {
			cerr << "Î´¼ì²âµ½author_rankÎÄ¼þ" << endl;
			return false;
		}
		node* root = NULL;//×ÖµäÊ÷Ê÷¸ù
		fstream source_file;
		source_file.open("database\\author_rank", ios::in);
		Build_Tree(source_file, root);
		Found_Tree(root, alls);
		sort(alls, 0, alls.size() - 1);
		fstream answer_file("database\\author_rank_result", ios::out);
...
namespace Keyword_Sorting {
	...
	void Keyword_Sort(DWORD year)
	{
		system("mkdir database\\year_sorted 2>nul");
		Author_Sorting::node* root;
		fstream iner("database\\year\\" + to_string(year) + ".ini", ios::in);
		...
		fstream outer("database\\year_sorted\\" + to_string(year) + ".db", ios::out);
		for (int i = 0; i < alls.size(); i++) {
